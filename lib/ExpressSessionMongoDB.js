        ...
        HandleError(Err, Callback, function() {
            Self.DB.collection(Self.CollectionName, function(Err, SessionCollection) {
                HandleError(Err, Callback, function() {
                    ...
                    {
                        var Now = new Date();
                        if(SessionCollection.findOneAndUpdate) //Method on 2.x.x mongodb driver, much faster than findAndModify
                        {
                            SessionCollection.findOneAndUpdate({'SessionID': SessionID }, {'$set': {'LastAccessed': Now}}, {'returnOriginal': false}, function(Err, Session) {
                                HandleError(Err, Callback, function() {
                                    GetSessionInfo.call(Self, Session.value, SessionCollection, SessionID, Callback);
                                });
                            });
                        }
                        else
